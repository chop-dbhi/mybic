...
def merge_commit(commit):
    ...
    with cd(env.path):
        ...
        run('find . -type f | grep .pyc | xargs rm')
...
def symlink_nginx():
    ...
    with cd(env.path):
        run('ln -sf $PWD/server/nginx/{host}.conf '
            '{nginx_conf_dir}/mybic-{host}.conf'.format(**env))
...
def reload_supervisor():
    ...
    with cd(env.path):
        run('ln -sf $PWD/server/supervisor/{host}.ini '
            '{supervisor_conf_dir}/mybic-{host}.ini'.format(**env))
...
def reload_wsgi():
    ...
    pid = run('supervisorctl pid mybic-{0}'.format(env.host))
    run('kill -HUP {0}'.format(pid))
...
def upload_settings():
    ...
    if os.path.exists(local_path):
        remote_path = os.path.join(env.path, 'mybic/conf/local_settings.py')
